...
import { signInWithEmailAndPassword, sendPasswordResetEmail } from 'firebase/auth'; // Import Reset
import { auth } from '../firebase';
import { Lock, Mail, ArrowLeft, AlertCircle, Loader2, KeyRound, CheckCircle2 } from 'lucide-react'; // Icon Tambahan

const LoginPage: React.FC = () => {
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [error, setError] = useState('');
    const [successMsg, setSuccessMsg] = useState(''); // State pesan sukses reset
    const [loading, setLoading] = useState(false);

    // STATE BARU: Mode Reset Password
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSuccessMsg(`Link reset password telah dikirim ke ${email}. Silakan cek kotak masuk atau spam email Anda.`);
        } catch (err: any) {
            console.error(err);
            if (err.code === 'auth/user-not-found') {
                setError('Email tidak terdaftar dalam sistem.');
            } else {
                setError('Gagal mengirim link reset. Pastikan email benar.');
            }
        } finally {
            setLoading(false);
        }
        ...
        <div className="min-h-screen flex font-sans bg-white">
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-6 md:p-12 bg-slate-50 lg:bg-white relative">
                ...
                <div className="w-full max-w-md bg-white lg:bg-transparent p-8 lg:p-0 rounded-3xl shadow-xl lg:shadow-none border border-slate-100 lg:border-none animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <div className="mb-10 text-center lg:text-left">
                        ...
                        <p className="text-slate-500">
                            {isResetMode
                                ...
                                : 'Silakan login untuk mengakses panel administrator.'}
                        </p>
                    </div>
                    ...
                    {/* Alert Sukses Reset */}
                    {successMsg && (
                        <div className="mb-6 p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex items-start text-emerald-700 text-sm animate-in zoom-in duration-300">
                    ...
                    <form onSubmit={isResetMode ? handleResetPassword : handleLogin} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center ml-1">
                                    ...
                                    <button
                                        ...
                                        className="text-xs font-bold text-[#0D5C35] hover:underline"
                                    >
                                        Lupa Password?
                                    </button>
                                </div>
                        ...
                        {isResetMode && (
                            <button
                                ...
                                className="w-full py-3 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 transition-all mt-4"
                            >
                                Kembali ke Login
                            </button>
                        )}
